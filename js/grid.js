...
var Grid = (function() {
    ...
    var fullimage  = $('[data-largesrc]').attr('data-largesrc');
    ...
    Preview.prototype = {
        ...
        update : function( $item ) {
            ...
            if( self.$videosrc.is( ':visible' ) ) {
                this.$loading.show();
                $( '<video/>' ).load( function() {
                    var $video = $( this );
                    if( $video.attr( 'src' ) === self.$item.children('a').data( 'video' ) ) {
                        self.$loading.hide();
                        self.$videosrc.find( 'video' ).remove();
                        self.$largevideo = $video.fadeIn( 350 );
                        self.$videosrc.append( self.$largevideo );
                    } else {
                        console.log('what?');
                    }
                } ).attr( 'src', eldata.video );
            }
